...
process subsampling_fastqs {
	...
	output:
		set val(smpl_id) , file("${smpl_id}_read1_sub.fastq.gz"), file("${smpl_id}_read2_sub.fastq.gz") into reads_starfusion, reads_fusioncatcher, reads_jaffa, reads_align, reads_salmon, reads_fastqscreen, reads_meta
...
process import_to_coyote {
	...
	script:
		id= "${smpl_id}-fusions-testDownsampling"
		group= 'fusion'
