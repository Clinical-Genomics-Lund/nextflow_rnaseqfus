...
GetOptions( \%opt, 
	    ...
	    'bam-file=s', 'vcf-desc=s', 'vcf-file', 'overwrite', 'assay=s', 'desc=s', 'subassay=s',
            'family=s', 'qc=s', 'sex=s', 'contamination=s', 'asmqc=s' );
...
# Add denovo assembl QC-data
if( $opt{'asmqc'} ) {
    add_asmqc( $opt{'sample-id'}, $opt{'run-folder'}, $opt{'assay'}, $opt{'asmqc'} );
}
...
sub add_asmqc {
    my( $id, $run, $assay, $quast_fn) = @_;

    my %qc;
    my $line_count = `wc -l $quast_fn |cut -f 1 -d" "`;
    chomp $line_count;
    ...
    my $results = $DB_FILES->find_one( {'sample-id'=>$id, 'run'=>$run } );
    if( $results->{'asmqc'} and !$OVERWRITE ) {
	print STDERR "ERROR: Assembly QC data already exists. Use --overwrite\n";
	exit;	
    }
    else {
	$DB_FILES->update_one( {'sample-id'=>$id, 'run'=>$run, 'assay'=>$assay }, { '$set'=>{'asmqc'=>\%qc }}, {'upsert'=>1} );
    }
}
